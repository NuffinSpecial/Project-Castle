...
def parse_arguments(argv: List[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("sentence", nargs="*", help="Sentence(s) to translate. If omitted the input file must be provided.")
    parser.add_argument("-f", "--file", type=Path, help="Path to a text file with one sentence per line.")
...
def load_sentences(args: argparse.Namespace) -> List[str]:
    ...
    if args.file:
        ...
        sentences.extend([line.strip() for line in args.file.read_text(encoding="utf-8").splitlines() if line.strip()])
